...
def run_program(environment_temperature, environment_humidity, sun_radiation, personnel_density, cold_load_1,
                cold_load_24, heat_load_1, heat_load_24, chilled_water_temperature, heating_water_temperature,
                ashp_heat_source_water_temperature):
    """执行程序"""
    # 实例化一个全局常量类
    gc = Global_Constant()
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # 负荷预测的输入参数
        environment_temperature = random.randint(-4, 29)
        environment_humidity = random.randint(2, 15)
        sun_radiation = random.randint(0, 1000)
        personnel_density = random.randint(0, 1)
        cold_load_1 = random.randint(0, 140)
        cold_load_24 = random.randint(0, 140)
        heat_load_1 = random.randint(0, 61)
        heat_load_24 = random.randint(0, 61)

        # 执行程序
        run_program(environment_temperature, environment_humidity, sun_radiation, personnel_density, cold_load_1,
                    cold_load_24, heat_load_1, heat_load_24, chilled_water_temperature, heating_water_temperature,
                    ashp_heat_source_water_temperature)
