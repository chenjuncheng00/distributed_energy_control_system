...
def print_heating_season(ans, ice1, ice2, lb1_wp_heating_water, lb2_wp_heating_water, lb1_wp_hot_water, lb2_wp_hot_water, chph1, chph2,
    ...
    if chph1_load_ratio > 0:
        ...
        # 写入数据库
        wtd.write_to_database_chp1(syncbase, False, False, True, chp1_wp_heat_water_frequency, chp1_wp_source_water_frequency,
    ...
    if chph2_load_ratio > 0:
        ...
        # 写入数据库
        wtd.write_to_database_chp2(syncbase, False, False, True, chp2_wp_heat_water_frequency, chp2_wp_source_water_frequency,
    ...
    # 写入数据库
    wtd.write_to_database_station_in_out(syncbase, cold_out_total, heat_out_total, electricity_generation_total,
                                         lb_cold_out_total, lb_heat_out_total, lb_hot_water_out_total, cc_cold_out_total, chp_cold_out_total,
                                         chp_heat_out_total, ashp_cold_out_total, ese_cold_out_total, ese_heat_out_total,
                                         ice_electricity_out_total, photovoltaic_electricity_out_total,
                                         wind_electricity_out_total, accumulator_electricity_out_total,
    ...
    # 写入数据库
    wtd.write_to_database_income_cost(syncbase, ice_income_total, lb_cold_income_total, lb_heat_income_total,
                                      lb_hot_water_income_total, cc_cold_income_total, chp_cold_income_total, chp_heat_income_total, ashp_cold_income_total,
                                      ngb_hot_water_income_total, photovoltaic_income_total, wind_income_total, ice_cost_total, lb_cold_cost_total,
                                      lb_heat_cost_total, lb_hot_water_cost_total, cc_cold_cost_total, chp_cold_cost_total,
                                      chp_heat_cost_total, ashp_cold_cost_total, ashp_heat_cost_total, ngb_hot_water_cost_total)
    ...
    # 批量写入24小时折现图数据
    wtd.write_to_database_heat_actual_24(syncbase, datetime.datetime.now(), gc.period)
    ...
    wtd.write_to_database_cc_electricity_consumption_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_chp_heat_electricity_consumption_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_ashp_electricity_consumption_24(syncbase, datetime.datetime.now(), gc.period)
    ...
    wtd.write_to_database_lb_heat_out_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_chp_heat_out_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_ese_heat_out_24(syncbase, datetime.datetime.now(), gc.period)
    ...
    wtd.write_to_database_lb_heat_income_total_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_lb_hot_water_income_total_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_chp_heat_income_total_24(syncbase, datetime.datetime.now(), gc.period)
    ...
    wtd.write_to_database_lb_heat_cost_total_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_lb_hot_water_cost_total_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_chp_heat_cost_total_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_ashp_heat_cost_total_24(syncbase, datetime.datetime.now(), gc.period)
    ...
    wtd.write_to_database_lb_heat_efficiency_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_lb_hot_water_efficiency_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_chp_heat_cop_24(syncbase, datetime.datetime.now(), gc.period)
    wtd.write_to_database_ashp_heat_cop_24(syncbase, datetime.datetime.now(), gc.period)
